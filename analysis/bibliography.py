...
def _load_single_source(loader_func, file_path: pathlib.Path, name: str) -> Optional[litstudy.DocumentSet]:
...
def refine_with_scopus(docs: litstudy.DocumentSet) -> litstudy.DocumentSet:
    ...
    try:
        ...
        logger.info(f"Scopus refinement results: {len(docs_final)} found, {len(docs_notfound)} not found.")
        return docs_final
...
def analyze_stats_plots(docs: litstudy.DocumentSet, save: bool, output_dir: pathlib.Path):
    ...
    plot_ops = [
        ...
        (litstudy.plot_affiliation_histogram, {'limit': PLOT_LIMIT_BARS}, 'affiliation_histogram.png'),
        (litstudy.plot_author_histogram, {'limit': PLOT_LIMIT_BARS}, 'author_histogram.png'), # Added limit
...
def analyze_topics(docs: litstudy.DocumentSet, topic_keyword: str, save: bool, output_dir: pathlib.Path) -> litstudy.DocumentSet:
